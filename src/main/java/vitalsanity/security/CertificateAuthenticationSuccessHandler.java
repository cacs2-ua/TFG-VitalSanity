...
public class CertificateAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        if (usuario.getTipo() != null) {
            ...
            if (tipoId == 1) {
                response.sendRedirect("https://192.168.43.218:8058/vital-sanity/api/admin/check");
                return;
            }
            if (tipoId == 2) {
                response.sendRedirect("https://192.168.43.218:8058/vital-sanity/api/centro-medico/check");
                return;
            }
            if (tipoId == 3) {
                response.sendRedirect("https://192.168.43.218:8058/vital-sanity/api/profesional-medico/5/pacientes-que-han-autorizado");
                return;
            }
            if (tipoId == 4) {
                response.sendRedirect("https://192.168.43.218:8058/vital-sanity/api/paciente/check");
                return;
            }
        }
        response.sendRedirect("https://192.168.43.218:8058/vital-sanity/api/auth/check");
    }
}